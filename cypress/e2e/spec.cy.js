describe('Url Shortener', () => {
  ...
  it('Displays homepage', () => {
    ...
    .get('.submit-button').contains('Shorten Please!')
    ...
    .get('.url').first().contains('.delete-button', '🪄')
    ...
    .get('.url').last().contains('.delete-button', '🪄')
  })
  ...
  it('Deletes cards', () => {
    ...
    cy.intercept('GET', 'http://localhost:3001/api/v1/urls', {
      statusCode: 200,
      fixture: 'newUrls'
    }).as('getUrlAfterDeletion')
    .get('.delete-button').first().click()
    .get('.url-container').children().should('have.length', 1)
    .get('.url').first().contains('h3', 'shopping')
    .get('.url').first().contains('a', 'http://localhost:3001/useshorturl/2')
    .get('.url').first().contains('p', 'https://youngdays.com/products/saint-james-onesie?variant=44242126930160&currency=USD&utm_medium=product_sync&utm_source=google&utm_content=sag_organic&utm_campaign=sag_organic&gad_source=1&gclid=Cj0KCQjwlZixBhCoARIsAIC745DlnoARAsD2V9ZJ1YZ5YajquEz0lK2AAwWEuHXU3lT6-KQ5laAComwaAn4qEALw_wcB')
    .get('.url').first().contains('.delete-button', '🪄')
